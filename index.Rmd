...
names(tap)<-c("No","Matricula","Nombre", "parcial1","parcial2","parcial3", "promedio")
tap <- tap %>%
  mutate(across(starts_with("parcial"), as.numeric))
tap <- tap %>%
  mutate(promedio = rowMeans(select(., starts_with("parcial")), na.rm = TRUE))
head(tap)
